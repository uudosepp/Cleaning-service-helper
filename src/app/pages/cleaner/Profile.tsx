...
export function CleanerProfile() {
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    setSaving(true);
    try {
      await profilesService.updateSelf({ phone });
  ...
  return (
    <div>
      ...
      <div className="max-w-lg space-y-4">
        ...
        <Card className="bg-card border-border">
          <CardHeader>
            <CardTitle className="text-sm">{t('profile_my_data')}</CardTitle>
          </CardHeader>
          <CardContent>
            <form onSubmit={handleSave} className="space-y-4">
              <div>
                <Label className="text-xs text-zinc-400">{t('name')}</Label>
                <Input value={profile?.full_name || ''} disabled className="mt-1 bg-input-background border-input opacity-50" />
              </div>
              <div>
              ...
              <div>
                <Label className="text-xs text-zinc-400">{t('organisation')}</Label>
                <Input value={profile?.tenants?.name || ''} disabled className="mt-1 bg-input-background border-input opacity-50" />
              </div>
              <div>
                <Label className="text-xs text-zinc-400">{t('phone')}</Label>
                <Input value={phone} onChange={(e) => setPhone(e.target.value)} placeholder="+372 555 1234" className="mt-1 bg-input-background border-input" />
              </div>
        ...
        <Card className="bg-card border-border">
          <CardHeader>
            <CardTitle className="text-sm text-white">{t('settings_change_email')}</CardTitle>
          </CardHeader>
          <CardContent>
        ...
        <Card className="bg-card border-border">
          <CardHeader>
            <CardTitle className="text-sm text-white">{t('settings_change_password')}</CardTitle>
          </CardHeader>
          <CardContent>
        ...
        <Button variant="outline" onClick={signOut} className="w-full text-red-400 border-red-400/30 hover:bg-red-400/10">
          {t('nav_sign_out')}
        </Button>
      </div>
    </div>
